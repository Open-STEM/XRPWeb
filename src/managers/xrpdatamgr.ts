...
export class XRPDataMgr {
                    ...
                    if (values) {
                        ...
                        if (this.READ_UNTIL_STRING == '') {
                            ...
                            if (this.SPECIAL_FORCE_OUTPUT_FLAG) {
                                if (this.CATCH_OK) {
                                    const v = this.TEXT_DECODER.decode(values);
                                    if (v.startsWith('OK')) {
                                        this.CATCH_OK = false;
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
                ...
                if (err.name == 'NetworkError') {
                    if (this.DEBUG_CONSOLE_ON)
                        console.log('%cDevice most likely unplugged, handled');
                    //this.disconnect();
    ...
    startBLEData() {
        ...
        this.READBLE!.addEventListener('characteristicvaluechanged', (event) => {
            ...
            if (this.BLE_DATA == undefined) {
                ...
                this.BLE_DATA = this.concatUint8Arrays(
                    this.BLE_DATA,
    ...
    public async writeToDevice(str: string | Uint8Array) {
        if (this.WRITER != undefined) {
            ...
            if (this.DEBUG_CONSOLE_ON)
                console.log('%cNot writing to device, none connected', 'color: red');
        }
    }
    ...
    async waitUntilOK() {
        ...
        while (this.DISCONNECT == false) {
            const tempLines = this.COLLECTED_DATA.split('\r\n');
    ...
    async haltUntilRead(omitOffset: number = 0, waitTime: number = -1): Promise<string[]> {
        ...
        while (this.DISCONNECT == false && numTimes != 0) {
            ...
            for (let i = 0; i < tempLines.length; i++) {
                if (
                    tempLines[i] == this.READ_UNTIL_STRING ||
                    this.READ_UNTIL_STRING == '' ||
                    tempLines[i].indexOf(this.READ_UNTIL_STRING) != -1 ||
                    tempLines[i] == '>'
                ) {
                    // Keyboard interrupt
